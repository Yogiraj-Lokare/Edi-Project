...
app.post("/view", auth, async (req, res) => {
  ...
  for (var i = 0; i < testData.list.length; i++) {
    var question = {
      key: testData.list[i]._id,
      no: i + 1,
      question: testData.list[i].question,
      mcq: testData.list[i].mcqs,
      marks: testData.list[i].marks,
      answer: testData.list[i].answer,
    };
    questionList.push(question);
  }
  const responseDataFormat = {
    test_name: testData.test_name,
    test_start: `${testData.start_time.date} @ ${testData.start_time.time}`,
    test_end: `${testData.end_time.date} @ ${testData.end_time.time}`,
    test_type: testData.test_type,
    list: questionList,
    allowed_users: testData.allowed_users,
  };
  return res.json(responseDataFormat);
});
